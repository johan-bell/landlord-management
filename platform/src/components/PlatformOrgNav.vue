...
const active = computed(() => ({
    ...
    support: name.value === 'organization-support',
}));
</script>

<template>
    <div class="mb-8 flex flex-wrap gap-2 border-b border-slate-200/80 pb-4">
        <RouterLink :class="cls(active.overview)" :to="base"
            >Overview</RouterLink
        >
        <RouterLink :class="cls(active.team)" :to="`${base}/team`"
            >Team</RouterLink
        >
        ...
        >
        <RouterLink :class="cls(active.support)" :to="`${base}/support`"
            >Support</RouterLink
        >
    </div>
</template>
