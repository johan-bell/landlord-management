...
function cls(active: boolean) {
  return [
    ...
    active ? 'bg-indigo-100 text-indigo-900' : 'text-slate-600 hover:bg-white/80 hover:text-slate-900',
  ];
}
...
const active = computed(() => ({
  ...
  properties:
    name.value === 'organization-properties' || name.value === 'organization-property-units',
...
<template>
  <div class="mb-8 flex flex-wrap gap-2 border-b border-slate-200/80 pb-4">
    ...
    <RouterLink :class="cls(active.properties)" :to="`${base}/properties`">Properties</RouterLink>
    <RouterLink :class="cls(active.renters)" :to="`${base}/renters`">Renters</RouterLink>
    <RouterLink :class="cls(active.leases)" :to="`${base}/leases`">Leases</RouterLink>
    <RouterLink :class="cls(active.payments)" :to="`${base}/payments`">Payments</RouterLink>
    <RouterLink :class="cls(active.signups)" :to="`${base}/signups`">Tenant signups</RouterLink>
    <RouterLink
