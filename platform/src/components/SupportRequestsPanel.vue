...
import PlatformDataPane from './PlatformDataPane.vue';

const props = defineProps<{
    /** When set, list is restricted to this org (org context tab). */
...
const showOrgColumn = computed(
    () =>
        props.scopedOrganizationId == null || props.scopedOrganizationId === '',
);

function syncFleetFilterFromRoute() {
    const q = route.query.organizationId;
    fleetOrgFilter.value = typeof q === 'string' ? q : '';
}
...
async function load() {
    ...
    try {
        ...
        const oid = orgFilterForRequest.value;
...
<template>
    <div>
        <PlatformDataPane
            ...
            empty-message="No support requests yet."
            @retry="load"
        >
            <template #filters>
                <div class="mb-4 flex flex-wrap gap-3">
                    ...
                    <label
                        v-if="showOrgColumn"
                        class="flex min-w-[200px] flex-1 items-center gap-2 text-sm"
                    >
                        <span class="text-slate-600">Organization ID</span>
                        <input
                            v-model="fleetOrgFilter"
                            type="text"
                    ...
                    <table class="min-w-full text-left text-sm">
                        ...
                        <tbody class="divide-y divide-slate-100">
                            ...
                            >
                                ...
                                <td
                                    v-if="showOrgColumn"
                                    class="hidden max-w-[160px] px-4 py-3 text-xs sm:table-cell"
                                >
                                    <RouterLink
        ...
        >
            ...
            >
                ...
                >
                    ...
                    <div
                        v-if="detail.organization && showOrgColumn"
