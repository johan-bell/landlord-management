...
const preview = ref<{ organizationName: string; email: string; role: string } | null>(null);
...
<template>
  <div class="flex min-h-screen flex-col justify-center px-4 py-10">
    <div class="mx-auto w-full max-w-md rounded-2xl border border-slate-200 bg-white p-8 shadow-sm">
      <h1 class="text-xl font-semibold text-slate-900">Organization invite</h1>

      <div v-if="loading" class="mt-6 text-sm text-slate-500">Checking invitation…</div>
      ...
      <template v-else-if="preview">
        <p class="mt-4 text-sm text-slate-600">
          ...
          <span class="font-semibold text-slate-900">{{ preview.organizationName }}</span>
          as
        ...
        <p v-if="acceptError" class="mt-4 text-sm text-red-600">{{ acceptError }}</p>

        <div v-if="!auth.accessToken" class="mt-6 space-y-3">
          <p class="text-sm text-slate-600">Sign in or register with <strong>{{ preview.email }}</strong>, then return here to accept.</p>
          <RouterLink
